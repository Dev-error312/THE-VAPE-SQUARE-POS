...
export default function ProductGrid({ products, loading }: ProductGridProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 space-y-2 border-b border-slate-800 bg-slate-900/50">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
          <input
            className="input pl-9 py-2 text-sm bg-slate-900"
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5">
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0 ${
                !selectedCategory ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700'
              }`}>
              All
            </button>
            {categories.map(cat => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0 ${
                  selectedCategory === cat ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700'
                }`}>
                {cat}
              </button>
            ))}
            ...
            {filtered.map(product => {
              ...
              return (
                <button
                  ...
                  disabled={atMax}
                  className={`relative bg-slate-800 border rounded-xl p-3 text-left transition-all ${
                    atMax
                      ? 'border-slate-700 opacity-50 cursor-not-allowed'
                      : 'border-slate-700 hover:border-primary-500/60 hover:bg-slate-700/80 active:scale-95 cursor-pointer'
                  }`}
                >
                  {/* Cart quantity badge */}
                  {inCart > 0 && (
                  ...
                  <p className="font-semibold text-slate-200 text-sm leading-snug line-clamp-2 mb-1 pr-6">
                    {product.name}
                  </p>
